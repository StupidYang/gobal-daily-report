...
当前自动更新暂停；合成演示位于 [demo/](demo/)。[修复与调度审计](docs/repair-20260922.md) 记录真实完成边界、三个外部任务和恢复条件。测试数据不是实时行情，不能用作交易信息。下方早期架构说明中的独立七任务调度已由 atomic-batch-v2 替代，七个模块仅保留为内部职责。
