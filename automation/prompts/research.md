...
从config.usPools、用户仓库自选及A/H/US榜单已入选证券发现研究需求。每次先查新材料身份，最多新分析config.research.maxNewAnalysesPerRun家公司，默认5；其余pendingQueue按重要性和等待时间排序。没有新材料则沿用旧研究，保留原analyzedAt；只更新实际完成的检查checkedAt。
