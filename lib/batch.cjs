...
const GROUPS={'global-main':P.W.MODULES,'asia-session':['asia-equities','synthesis'],'us-session':['us-equities','synthesis']};
...
function prepareBatch(root,batch,now){
  ...
  try {
    for(const rel of [...outputRoots,'automation/control.json','config/watchlist.json',Control.MARKER]){
...
function promoteBatches(root,now=Date.now(),write=P.atomic){
  const control=Control.readControl(root);if(control.productionPaused&&!Control.isFixtureRoot(root))return [{status:'paused',writes:0}];
