...
function checkMode(root, candidate) {
  ...
  const validation=candidate?.validationOnly===true||candidate?.payload?.report?.reportMeta?.validationOnly===true;
  if(validation!==isValidationRoot(root))return ['Real-data validation candidates and production roots must remain isolated'];
  return [];
}
module.exports = { MARKER, VALIDATION_MARKER, isFixtureRoot, isValidationRoot, isIsolated, readControl, checkMode };
