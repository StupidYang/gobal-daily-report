...
async function begin(store,group,options){
 const snapshot=await store.read(),state=acquire(snapshot.state,group,options);
 try{return await store.cas(snapshot,state);}catch(error){
  // Read back once: an uncertain write may have succeeded, or a peer won the CAS.
  let actual;try{actual=await store.read();}catch{throw error;}
  if(actual.state.executionId===state.executionId&&actual.state.generation===state.generation)return actual;
  const held=actual.state.phase==='publishing'||ACTIVE.has(actual.state.phase)&&ms(actual.state.deadlineAt)>(options?.now??Date.now());
  if(error.code==='CONFLICT'&&held&&actual.state.executionId!==state.executionId)throw new Busy('Another execution won the lease; skip without collecting');
  throw error;
 }
}
