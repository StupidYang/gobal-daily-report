...
function prepareMacro(previous,input={},now=Date.now()){
 ...
 const previousTime=previous?P.W.time(previous.generatedAt):null,age=previousTime===null?Infinity:now-previousTime;
 if(!hasNew&&previous&&age>=0&&age<=24*3600000){
  const retention={runId:previous.runId,generatedAt:previous.generatedAt,dataAsOf:previous.dataAsOf??null,reason:'本轮未取得新的合格宏观/资金证据；保留上一份24小时内来源支持的模块，所有原时点保持不变。'};
  ...
  payload.coverage={...(payload.coverage||{}),status:'partial',retained:true,retainedFrom:previous.runId,note:[payload.coverage?.note,retention.reason].filter(Boolean).join('；')};
