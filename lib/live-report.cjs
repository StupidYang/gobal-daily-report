...
function prepareMacro(previous,input={},now=Date.now()){
 const macroFacts=input.macroFacts||[],macroEvents=input.macroEvents||[],events=input.events||[],fundingNotes=input.macroFundingNotes||[];
 const hasNew=[macroFacts,macroEvents,events,fundingNotes].some(x=>Array.isArray(x)&&x.length);
 // A new wrapper is not new evidence. Preserve the original retention anchor.
 const previousTime=previous?P.W.time(previous.retention?.generatedAt??previous.generatedAt):null,age=previousTime===null?Infinity:now-previousTime;
 if(!hasNew&&previous&&age>=0&&age<=24*3600000){
  const retention=previous.retention?clone(previous.retention):{runId:previous.runId,generatedAt:previous.generatedAt,dataAsOf:previous.dataAsOf??null,reason:'本轮未取得新的合格宏观/资金证据；保留上一份24小时内来源支持的模块，所有原时点保持不变。'};
  const payload=clone(previous.payload||{canonicalFacts:[],macroEvents:[],events:[]});
  for(const key of ['canonicalFacts','macroEvents','events'])if(Array.isArray(payload[key]))payload[key]=payload[key].map(x=>x&&typeof x==='object'?{...x,retention:x.retention||retention}:x);
  payload.coverage={...(payload.coverage||{}),status:'partial',retained:true,retainedFrom:retention.runId,note:[...new Set([...(payload.coverage?.note||'').split('；'),retention.reason].filter(Boolean))].join('；')};
  return {status:'no-change',dataAsOf:previous.dataAsOf??null,payload,retention,retainedSources:clone(previous.sources||[])};
 }
 return {status:'partial',dataAsOf:null,payload:{canonicalFacts:macroFacts,macroEvents,events,fundingNotes,coverage:input.macroCoverage||{status:'partial',note:hasNew?'本轮宏观/资金内容仅包含已提交的来源支持项目。':'本轮没有新的合格宏观/资金证据，且上一模块超过24小时保留窗口。'}},retention:null,retainedSources:[]};
}
...
function compile(root,packet,editorial,options){
 ...
 for(const s of macroPrepared.retainedSources){const old=allSources.get(s.id);if(old&&old.url!==s.url)throw Error('Macro source ID collision while retaining previous evidence');allSources.set(s.id,s);}
...
module.exports={sourceId,sourcesFrom,preparedInputs,snapshotPeriod,prepareMacro,assertEventRadar,compile,minute,utc8};
