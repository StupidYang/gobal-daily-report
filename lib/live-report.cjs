...
function compile(root,packet,editorial,options){
 ...
 modules.research.sources=[...allSources.values()];modules.research.payload={records:e.researchRecords||[],checks:e.researchChecks||[],pendingQueue:Object.values(config.usPools).flat().filter(symbol=>!(e.researchRecords||[]).some(x=>x.instrumentId==='EQUITY:US:'+symbol)).map(symbol=>({instrumentId:'EQUITY:US:'+symbol,reason:'本轮没有新完成此公司的原文财报研究；不把报价检查冒充基本面研究。'}))};
