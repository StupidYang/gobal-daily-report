...
function compile(root,packet,editorial,options){
 ...
 const previousMacro=P.read(path.join(root,'data/modules/macro.json')),macroFacts=e.macroFacts||[],macroEvents=e.macroEvents||[],macroEventRadar=e.events||[],macroFunding=e.macroFundingNotes||[];
 const hasNewMacro=[macroFacts,macroEvents,macroEventRadar,macroFunding].some(x=>Array.isArray(x)&&x.length);
 const previousMacroAge=previousMacro&&P.W.time(previousMacro.generatedAt)!==null?now-P.W.time(previousMacro.generatedAt):Infinity;
 if(!hasNewMacro&&previousMacro&&previousMacroAge>=0&&previousMacroAge<=24*3600000){
  for(const s of previousMacro.sources||[]){const old=allSources.get(s.id);if(old&&old.url!==s.url)throw Error('Macro source ID collision while retaining previous evidence');allSources.set(s.id,s);}
  const retention={runId:previousMacro.runId,generatedAt:previousMacro.generatedAt,dataAsOf:previousMacro.dataAsOf??null,reason:'本轮未取得新的合格宏观/资金证据；保留上一份24小时内来源支持的模块，所有原时点保持不变。'};
  const payload=clone(previousMacro.payload||{canonicalFacts:[],macroEvents:[],events:[]});
  ...
  payload.coverage={...(payload.coverage||{}),status:'partial',retained:true,retainedFrom:previousMacro.runId,note:[payload.coverage?.note,retention.reason].filter(Boolean).join('；')};
  modules.macro.sources=[...allSources.values()];modules.macro.payload=payload;modules.macro.dataAsOf=previousMacro.dataAsOf??null;modules.macro.status='no-change';modules.macro.retention=retention;
 }else{
  modules.macro.sources=[...allSources.values()];modules.macro.payload={canonicalFacts:macroFacts,macroEvents,events:macroEventRadar,fundingNotes:macroFunding,coverage:e.macroCoverage||{status:'partial',note:hasNewMacro?'本轮宏观/资金内容仅包含已提交的来源支持项目。':'本轮没有新的合格宏观/资金证据，且上一模块超过24小时保留窗口。'}};
 }
