...
function merge(previous,incoming,{now=Date.now(),coverage,sources=[]}={}){
 ...
 for(const x of items)for(const region of new Set(arr(x.regions)))regionCounts[region]++;
 const general=items.filter(x=>x.kind==='general').length,market=items.length-general;
 const externalItems=items.filter(x=>evidenceClass(x,sourceMap)==='external'),externalVerified=externalItems.length;
 const externalRegionCounts={CN:0,US:0,WORLD:0};
 for(const x of externalItems)for(const region of new Set(arr(x.regions)))externalRegionCounts[region]++;
 const externalGeneral=externalItems.filter(x=>x.kind==='general').length;
