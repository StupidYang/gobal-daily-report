...
function mergeResearch(previous,incoming){
 ...
 const checks=new Map();for(const x of [...W.arr(previous?.payload?.checks),...W.arr(incoming.payload.checks)]){const key=[x?.instrumentId||'',x?.checkedAt||'',x?.eventKey||'',x?.status||x?.result||'',x?.documentId||''].join('|');checks.set(key,x);}
 const mergedChecks=[...checks.values()].sort((a,b)=>(W.time(a?.checkedAt)||0)-(W.time(b?.checkedAt)||0)).slice(-500);
 return {...incoming,sources:[...sources.values()],payload:{...incoming.payload,records:[...records.values()],checks:mergedChecks,cacheAudit:{added,reused,checksRetained:Math.max(0,mergedChecks.length-W.arr(incoming.payload.checks).length),note:'同事件沿用原分析及analyzedAt；研究检查保留实际checkedAt；新财报/指引/重述/研报需新eventKey。'}}};
}
...
function ingest(root,m,now=Date.now()){
 ...
 return locked(root,()=>{
  ...
  if(['quotes','asia-equities','us-equities'].includes(m.module)&&previous&&W.time(previous.generatedAt)<=W.time(m.generatedAt)){m=require('./retention.cjs').merge(previous,m);const errors=W.validate(m,m.module,now);if(errors.length)throw Error(errors.join('\n'));}
...
function publishReport(root,report){
 const e=validateReport(report);if(read(path.join(root,'automation/control.json'))?.qualityPolicy==='content-r3')e.push(...require('../assets/content-contract.js').quality(report,read(path.join(root,'data/latest.json'))).errors);if(e.length)throw Error(e.join('\n'));
 return locked(root,()=>{
...
function context(root,role){
 const manifest=read(path.join(root,'automation/manifest.json')),task=(manifest.roles||manifest.tasks).find(t=>t.role===role);if(!task)throw Error('未知角色 '+role);
