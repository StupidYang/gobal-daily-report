...
function mergeResearch(previous,incoming){
 ...
 const reused=[],added=[];
 W.arr(incoming.payload.records).forEach(r=>{const key=r.instrumentId+'|'+r.eventKey;if(records.has(key)){reused.push(key);return;}records.set(key,r);added.push(key);});
...
function ingest(root,m,now=Date.now()){
 const e=W.validate(m,m.module,now);if(e.length)throw Error(e.join('\n'));
 return locked(root,()=>{
  ...
  if(existing?._inputHash===inputHash)return {status:'idempotent',path:archivePath(m)};
  if(m.module==='research'){m=mergeResearch(previous,m);const errors=W.validate(m,'research',now);if(errors.length)throw Error(errors.join('\n'));}
...
function compile(root,role,entry=false){
 const manifest=read(path.join(root,'automation/manifest.json')),task=manifest.tasks.find(t=>t.role===role);if(!task)throw Error('未知角色 '+role);
 if(entry)return `执行GDR模块 ${role}。仓库：${process.env.GDR_REPO||manifest.repository}。先读取 automation/manifest.json、automation/prompts/common.md、${task.promptFile}、docs/modules-contract.md 和 config/watchlist.json；按这些版本化文件执行。只读本角色依赖，只写manifest规定的ownedPaths，禁止写其他模块、前端、配置、提示词。${role==='synthesis'?'你是综合报告唯一发布者，保留每4小时完整报告、全部时事与研究。':'你是数据生产者，不得覆盖data/latest.json或报告历史索引。'}联网核验，不编行情/排名/研报；无数据明确缺失，失败不假装成功。最终说明模块时点、采集与覆盖缺口、实际GitHub写入及回读结果。调度已独立配置，不创建或修改任务。`;
