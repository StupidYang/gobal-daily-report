...
const W=P.W,MAX_BYTES=4*1024*1024;
const utc8=t=>new Date(t+8*3600000).toISOString().replace('Z','+08:00');
function references(root,report,now){
 const errors=[];
 for(const [role,ref]of Object.entries(report?.reportMeta?.moduleRefs||{})){
  ...
  if(W.time(m.generatedAt)>W.time(report.updatedAt))errors.push('冻结模块位于报告未来 '+role);
...
function preflight(root,m,role,now){
 const errors=W.validate(m,role,now);
 if(role==='synthesis'){
  errors.push(...P.validateReport(m?.payload?.report));
  if(!errors.length){const r=m.payload.report;if(r.reportMeta?.contractVersion!=='reader-r2')errors.push('新综合报告必须使用reader-r2完整内容契约');if(W.time(r.updatedAt)>W.time(m.generatedAt))errors.push('报告截止晚于模块生成时间');errors.push(...references(root,r,now));}
 }
 ...
 return P.locked(root,()=>{
  ...
  for(const item of queue){
   ...
   try{
    ...
    receipt.status=e.message.split('\n').every(s=>s.startsWith('DEPENDENCY_MISSING:'))&&receipt.attempts<5?'waiting-dependencies':'rejected';receipt.errors=[e.message];delete receipt.module;delete receipt.report;P.atomic(receiptPath,receipt);
   }
   result.push(receipt);
  }
