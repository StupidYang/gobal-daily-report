...
function build(root){
 ...
 for(const {report:r}of history){
  if(r.reportMeta?.dataMode==='synthetic')continue;const cut=P.W.time(r.reportMeta?.generatedAt||r.updatedAt);if(cut===null)continue;
