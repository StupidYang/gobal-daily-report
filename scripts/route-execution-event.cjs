...
function select(files){
 const scoped=files.filter(x=>/^runtime\/(requests|submissions)\/[A-Za-z0-9][A-Za-z0-9_-]{0,83}\.json$/.test(x.filename||''));
 if(scoped.length!==1||scoped[0].status!=='added')throw Error('Exactly one new immutable request or submission is required');
 const file=scoped[0].filename,mode=file.includes('/requests/')?'request':'submit';let id=file.split('/').pop().slice(0,-5);const revision=id.match(/--r(\d+)$/);
 if(revision){if(mode!=='submit'||revision[1]!=='1')throw Error('Only submission --r1 is a supported revision');id=id.slice(0,-revision[0].length);}
 if(id.length>80)throw Error('Execution ID is too long');return {file,mode,id,...(revision?{revision:1}:{})};
}
...
if(require.main===module){const store=new E.GitHubStore({repository:process.env.GITHUB_REPOSITORY});(async()=>{const event=JSON.parse(fs.readFileSync(process.env.GITHUB_EVENT_PATH,'utf8')),r=await route(event,store.request.bind(store));console.log(JSON.stringify({routing:'pinned-commit-api',...r}));cp.execFileSync(process.execPath,['scripts/execution-worker.cjs',r.mode,r.id,String(r.revision||0)],{stdio:'inherit',timeout:240000,env:{...process.env,GDR_HANDOFF_REF:event.after}});})().catch(e=>{console.error(e.stack);process.exitCode=1;});}
