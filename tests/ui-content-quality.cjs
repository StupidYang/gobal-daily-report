 ...
 try{
  ...
  for(const width of (process.env.GDR_TEST_WIDTHS||'390,1440').split(',').map(Number)){
   ...
   for(const c of cases){
    ...
    if(raw.reportMeta?.dataMode!=='synthetic'){const view=await read(context.request,'data/reader-projections/'+c.reportId+'.json');assert.ok(text.includes('历史沿用'),'Recovery disclosed');assert.equal(await page.locator('#mainlandIndices .cn-index-value').count(),6);assert.ok(!text.includes('8条事实 · 缺失 0'),'Misleading global health label');if(view.quality.errors.length)assert.ok(text.includes('未通过新的内容质量标准'));}
   ...
   await page.locator('#watchlist').evaluate(d=>d.open=true);await page.getByRole('button',{name:'板块热 / 弱榜',exact:true}).click();await page.getByLabel('榜单市场',{exact:true}).selectOption('US');await page.evaluate(()=>document.querySelectorAll('#watchlist details').forEach(d=>d.open=true));
   const watch=await page.locator('#watchlist').innerText();assert.ok(watch.includes('历史候选样本')||r.reportMeta?.dataMode==='synthetic','Historical sample origin must be visible');
