...
test('manifest has seven independent roles and only one global report writer',()=>{const m=require('../automation/manifest.json');assert.equal(m.tasks.length,7);assert.deepEqual(m.tasks.filter(t=>t.ownedPaths.includes('data/latest.json')).map(t=>t.role),['synthesis']);});
