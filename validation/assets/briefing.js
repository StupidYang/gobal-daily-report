...
function factLink(f){const a=E('a','metric-fact-link',D.name(f));a.href=f.projectionOf?'#watchlist':'#fact-'+encodeURIComponent(f.id);a.title='查看 '+D.name(f)+' 的来源和原始口径';return a;}
...
function mainland(r){
 ...
 function paint(module,status,description){
  ...
  for(const q of quotes){
   ...
   if(valid){
    ...
    card.append(E('span','cn-index-time',C.stamp(q.asOf)+' UTC+8'),E('span','cn-index-status',q.retention?'历史沿用 · 未重新核验':D.status(q.dataStatus||q.status)));
 ...
 if(ref){
  ...
  Promise.resolve().then(()=>{
   ...
   .then(m=>{frozenQuotes(r,m);const projected=r._readerProjection?.modules?.quotes;if(projected?.runId===m.runId)m=frozenQuotes(r,projected);if(panel.isConnected)paint(m,'ready','读取本报告绑定的行情快照；各指数保留原始数据时点');})
...
function cards(r){
 const block=E('div','briefing-market'),box=E('div','metrics readable-metrics'),groups=new Map(),all=C.facts(r).slice();
 const existing=new Set(all.map(f=>D.identity(f)));for(const q of C.arr(r._readerProjection?.modules?.quotes?.payload?.items)){if(!existing.has(q.instrumentId)&&D.hasValue(q)){all.push({...q,id:q.instrumentId,projectionOf:true});existing.add(q.instrumentId);}}
 ...
 order.filter(g=>g!=='CN'&&groups.has(g)).slice(0,6).forEach(g=>{
  ...
  const values=E('div','quote-stack');shown.forEach(f=>{const row=E('div','quote-line'),top=E('div','quote-line-top');top.append(factLink(f),E('strong','quote-number',D.format(f)));const bottom=E('div','quote-line-meta'),ch=D.change(f);bottom.append(E('span',ch.startsWith('-')?'negative':ch.startsWith('+')?'positive':'muted',ch),E('span','',C.parseTime(f.asOf)!==null?C.stamp(f.asOf):'时点待核'));row.append(top,bottom);if(f.retention)row.append(E('span','cn-index-status','历史沿用 · 未重新核验'));if(f.contract)row.append(E('span','quote-contract','合约 '+f.contract));values.append(row);});card.append(values);
