# 当前发布协议：atomic-batch-v2

实际外部调度为三个任务，内部保留七个模块。执行与提交以 automation/tasks/common.md 为准。新任务只提交 data/inbox/batches/<batchId>.json，包含完整synthesis；旧单模块候选接口仅为历史兼容。详见 docs/repair-20260922.md。
